...
| personal_id | 取票人身分證字號 | "A123456789" |
| email | 電子郵件（選填） | "example@email.com" |
| phone | 手機號碼（選填） | "0912345678" |
| tgo_account | TGo 會員帳號（選填） | "" |
| passenger_ids | 乘客身分證列表（選填，購買愛心/敬老票時需要） | ["B234567890", "C345678901"] |
| tickets | 各票種數量 | 見下方說明 |
...
#### 乘客身分證設定說明 (passenger_ids)

**重要**：購買敬老票、愛心票等優惠票種時，需要填寫乘客身分證字號。

**使用方式**：
- **留空 `[]`**：程式執行時會逐一詢問每位乘客的身分證
- **預先設定**：可在陣列中預填身分證，按購票順序填寫
  ```json
  "passenger_ids": ["B234567890", "C345678901"]
  ```
- **混合模式**：部分預設、部分詢問（推薦）
  - 若預設數量不足，程式會詢問缺少的部分

**注意事項**：
- 敬老票：每位乘客需填寫**本人身分證**（不可重複）
- 愛心票：填寫身心障礙證明持有人身分證（陪伴者可填同一個）
- 同一身分證號不能同時用於購買敬老票及愛心票
- 程式會自動檢查並警告身分證重複問題
